"use client";

import { useEffect } from "react";
import { Toaster } from "@/components/ui/toaster";
import { ScrollArea } from "@/components/ui/scroll-area";
import { useMobileDetect } from "./mobile-detector";
import { useRouter, usePathname } from "next/navigation";
import FoodListSidebar from "./food-list-sidebar";
import { FoodList } from "@/lib/types";

interface SidebarLayoutProps {
  children: React.ReactNode;
  foodLists: FoodList[];
}

export default function SidebarLayout({ children, foodLists }: SidebarLayoutProps) {
  const isMobile = useMobileDetect();
  const router = useRouter();
  const pathname = usePathname();

  useEffect(() => {
    if (isMobile !== null && !isMobile && pathname === "/notes") {
      router.push("/notes/michelin");
    }
  }, [isMobile, router, pathname]);

  const handleFoodListSelect = (foodList: FoodList) => {
    router.push(`/notes/${foodList.slug}`);
  };

  if (isMobile === null) {
    return null;
  }

  const showSidebar = !isMobile || pathname === "/notes";

  return (
    <div className="dark:text-white h-dvh flex">
      {showSidebar && (
        <FoodListSidebar
          foodLists={foodLists}
          onFoodListSelect={isMobile ? handleFoodListSelect : () => {}}
          isMobile={isMobile}
        />
      )}
      {(!isMobile || !showSidebar) && (
        <div className="flex-grow h-dvh">
          <ScrollArea className="h-full" isMobile={isMobile}>
            {children}
          </ScrollArea>
        </div>
      )}
      <Toaster />
    </div>
  );
}
